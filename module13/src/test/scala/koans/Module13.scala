...
class Module13 extends FunSuite with Matchers with StopOnFirstFailure with SeveredStackTraces {
  ...
  // into a set
  def stringToSetOfChars(s1: String): List[Set[Char]] = stringToChars(s1).map(Set(_))
  ...
  def comboSetsForSequences(sequences: List[String]): List[Set[Char]] = {
    val head :: tail = sequences
    val initialSet = stringToSetOfChars(head)
    tail.foldLeft(initialSet)((set, str) => combineZippedSetsAndString(set, str))
  }
  ...
  def partitionByFirst(numbers: List[Int]): (Int, List[Int], List[Int]) = {
    ...
    val (lower, remaining) = tail.partition(_ < head)
