...
class Module07 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  abstract class Engine extends RollingStock {
    ...
    def addedCars: String = cars.mkString(" and ")

    def pull: String = s"$name pulls ${addedCars}"

    val maxCars: Int

    def add (car: Car): Engine = {
      if (cars.length >= maxCars) {
        throw new IllegalStateException(s"Can't add new car. It's already $maxCars.")
      }
      cars += car
      this
    }
  ...
  class SteamEngine extends Engine {
    override val name = "Steam engine"
...
//    steamEngine.cars.size should be (2)
  ...
  class ShuntEngine extends Engine {
    ...
    override def pull: String = s"$name doesn't pull, it pushes ${addedCars}"
  }
